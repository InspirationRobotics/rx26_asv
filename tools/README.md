...
| `scripts/batt_watchdog.py` or any `CRSD_BATT_*` threshold | `python3 tools/scripts/batt_watchdog.py --dry-run` against the real pack FIRST — it logs what it would do and never contacts the power helper. Only then `sudo systemctl restart crsd-battwatch`. Needs safety review: it can power the boat off, and a threshold set for the wrong pack does that on a healthy battery |
