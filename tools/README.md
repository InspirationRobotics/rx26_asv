...
| `systemd/` | `crsd-mavproxy.service`, `crsd-container.service`, `crsd-livox.service` (templates; `__PLACEHOLDERS__` expanded by `setup/install_jetson_host.sh`) | The boot chain is ordered so MAVProxy owns the Pixhawk before any node starts. A power cycle brings the boat up with no typed commands. The LiDAR branch is deliberately **not** ordered behind MAVProxy — they share no device, and coupling them would make a LiDAR fault look like an autopilot fault. |
