...
| `lidar_view.py` | MID360 cloud as plan + elevation panels in a browser (:8081). `--frame raw\|body` is the LiDAR orientation check — see [docs/G2_lidar_orientation.md](../docs/G2_lidar_orientation.md) | The extrinsic is read from `crusader_params.yaml` when it is built, so the bench view and the clustering node cannot disagree. A check performed against different numbers than the node uses would be worse than no check. |
| `mjpeg_server.py` | Shared HTTP/MJPEG plumbing behind both viewers | Extracted rather than copied: two copies of the dead-stream and browser-closed handling drift, and only one of them gets the fix. |
