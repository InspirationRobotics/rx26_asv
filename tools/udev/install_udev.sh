...
# crsd-pixhawk, so actuator_node would have opened the autopilot's serial link
# at 9600 baud while MAVProxy owned it — a second Pixhawk owner, on a boat with
# live thrusters. It was only caught by eyeballing an `ls`. Not any more.
declare -A crsd_seen=()
collision=0
...
if (( collision )); then
  ...
  echo "       Rules generated by gen_udev_rules.py match by PHYSICAL PORT," >&2
  echo "       so they go wrong the moment anything is recabled." >&2
  exit 1
fi
